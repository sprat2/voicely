...
// Handle letters' meta display
//   Function is modeled after the one in /includes/lib/inc/functions.php
...
if ( ! function_exists( 'fusion_render_letter_metadata' ) ) {
	/**
	 ...
	 * @return  string
	 */
	// function fusion_render_letter_metadata( $layout, $settings = array() ) {
...
function add_addressees_to_search_params($query) {
    if ( !is_admin() && $query->is_main_query() && $query->is_search() && is_search() ) {
		...
		// 	$tax_query[] = $new_tax_query_params;
