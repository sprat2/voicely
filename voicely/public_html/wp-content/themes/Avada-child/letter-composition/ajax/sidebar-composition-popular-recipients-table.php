      ...
      for ($i = 0; $i < $numOfRows; $i++) {
        ...
        for ($j = $i * $numPerRow; ($j < ($i+1) * $numPerRow); $j++) {
          ...
          if ($j < sizeof($recipients)) {
            ...
            if ( $image_exists )
              echo "\t\t\t\t".'<img id="scroll-recipient-id-'.$current_term_id.'" src="'.$current_img_url.'" onclick=\'addresseeClicked("'.$current_term_id.'", "'.$current_term_name.'", "'.$current_prettyname.'")\' class="draggable tooltippable recipient">'."\n";
            else
              echo "\t\t\t\t".'<img id="scroll-recipient-id-'.$current_term_id.'" src="https://voicely.org/wp-content/themes/Avada-child/letter-composition/img/64x64.png" onclick=\'addresseeClicked("'.$current_term_id.'", "'.$current_term_name.'", "'.$current_prettyname.'")\' class="draggable tooltippable recipient">'."\n";
